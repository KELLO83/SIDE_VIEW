...
class YoloDetector:
    ...
    def prediction(self, img: Union[np.ndarray, torch.Tensor], flag: str) -> np.ndarray:
        try:
            ...
            if flag == 'cam0':
                img_res = self.LOCATION.cam0_run(img_res, nmx_boxes)
            elif flag == 'cam2':
                img_res = self.LOCATION.cam2_run(img_res, nmx_boxes)
    ...
    def set_run(self) -> None:
        ...
        try:
            ...
            for door_path, under_path in tqdm(self.set_list):
                ...
                under_prediction = self.prediction(under_plane, flag='cam2')
                door_prediction = self.prediction(door_plane, flag='cam0')
...
class Location():
        ...
        for i in boxes:
            ...
            if not (center_x >= 300):
                ...
                if left_x is not None and right_x is not None:
                    ...
                    position_info.append((idx, position))
    ...
    def cam0_run(self, img, boxes):
        ...
        for x1, _, _, _ in boxes:
            for idx in range(len(Location.cam0_x_limit) - 1):
                ...
                if lower_limit <= x1 <= upper_limit:
                    # Determine which limit x1 is closer to
        ...
        counter = Counter(people_where)
        counter = {4 - key: value for key, value in sorted(counter.items())}
        print("door : ", counter)
        
        for key, value in counter.items():
            loc = Location.cam0_x_limit[4 - key]
            cv2.putText(img, f"{value}", (loc + 30, 77), cv2.FONT_HERSHEY_DUPLEX, 2, (0, 0, 255))
